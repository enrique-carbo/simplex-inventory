...
import useCottonSwatersProductsStore from "@/app/store/cotton-swaters-store";
import type { CottonSweaterProduct } from "@/app/lib/definitions-pb";

const CottonSweatersPage: React.FC = () => {
  const { cottonSweatersProducts, setProducts, updateProductStock } =
  useCottonSwatersProductsStore();
  ...
  const fetchProducts = async () => {
    try {
      const response = await pb
        .collection("prod_buzos")
        .getFullList<CottonSweaterProduct>({
          expand: "BRAND_ID",
        });
      setProducts({
        page: 1,
      ...
      if (err instanceof Error) {
        setError(`Error al obtener los buzos: ${err.message}`);
      } else {
        setError("Error inesperado al obtener los buzos");
      }
    }
  };

  useEffect(() => {
    if (!productsLoaded && cottonSweatersProducts.items.length === 0) {
      fetchProducts();
    }
  }, [productsLoaded, cottonSweatersProducts.items.length]);
  ...
  const handleStockChange = async (productId: string, newStock: number) => {
    ...
    try {
      await pb.collection("prod_buzos").update(productId, {
        STOCK: newStock,
      });
      ...
      if (err instanceof Error) {
        ...
        const originalProduct = cottonSweatersProducts.items.find(
          (p) => p.id === productId
        );
        if (originalProduct) {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-900">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {cottonSweatersProducts.items.map((product) => (
              <tr
                key={product.id}
