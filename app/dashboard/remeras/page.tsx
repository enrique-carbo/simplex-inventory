...
const RemerasPage: React.FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await pb
        .collection("prod_remeras")
        .getFullList<RemerasProduct>({
          expand: "BRAND_ID",
        });
      setProducts({
        page: 1,
  ...
  useEffect(() => {
    if (!productsLoaded && remerasProducts.items.length === 0) {
      fetchProducts();
    }
  }, [productsLoaded, remerasProducts.items.length]);
  ...
  const handleStockChange = async (productId: string, newStock: number) => {
    ...
    try {
      await pb.collection("prod_remeras").update(productId, {
        STOCK: newStock,
      });
      ...
      if (err instanceof Error) {
        ...
        const originalProduct = remerasProducts.items.find(
          (p) => p.id === productId
        );
        if (originalProduct) {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-10">Remeras</h1>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-900">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {remerasProducts.items.map((product) => (
              <tr
                key={product.id}
