...
const PoloShirtsPage: React.FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await pb
        .collection("prod_chombas")
        .getFullList<PoloShirtProduct>({
          expand: "BRAND_ID",
        });
      setProducts({
        page: 1,
      ...
      if (err instanceof Error) {
        setError(`Error al obtener las chombas: ${err.message}`);
      } else {
        setError("Error inesperado al obtener las chombas");
      }
    }
  };

  useEffect(() => {
    if (!productsLoaded && poloShirtsProducts.items.length === 0) {
      fetchProducts();
    }
  }, [productsLoaded, poloShirtsProducts.items.length]);
  ...
  const handleStockChange = async (productId: string, newStock: number) => {
    ...
    try {
      await pb.collection("prod_chombas").update(productId, {
        STOCK: newStock,
      });
      ...
      if (err instanceof Error) {
        ...
        const originalProduct = poloShirtsProducts.items.find(
          (p) => p.id === productId
        );
        if (originalProduct) {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold mb-10">Chombas</h1>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-900">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {poloShirtsProducts.items.map((product) => (
              <tr
                key={product.id}
